...
- **Kuveyt Türk Identity:** resmi uç yalnızca `POST https://id[.prep].kuveytturk.com.tr/api/connect/token` — body’de `grant_type=client_credentials&client_id&client_secret&scope=public` (iuysal/Android SDK; HTTP Basic yok). `/connect/token` 404 HTML döner; bunu `invalid_client` sanma. `invalid_client` çoğunlukla Canlı↔Sandbox kimlik karışması veya Api Anahtarının Client Secret yerine yazılmasıdır — diğer Identity host’u teşhis için dene. Api Anahtarı token secret değildir (`X-Gravitee-Api-Key`). RSA-SHA256 Signature yalnızca API çağrılarında.
