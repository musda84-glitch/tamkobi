...
export default function TradePage() {
  ...
  const setStatus = async (r, status) => {
    try {
      await axios.post(`${API_URL}/trade-files/${r.id}/status`, { status });
      toast.success(statusTr(status)); load();
    } catch (e) { toast.error(e.response?.data?.detail || "Durum güncellenemedi."); }
  };
            ...
            {rows.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50" data-testid={`trade-row-${r.file_number}`}>
                ...
                <td className="px-4 py-3 text-right whitespace-nowrap">
                  ...
                  {canEdit && r.status === "draft" && <button onClick={() => setStatus(r, "declared")} className="px-2 py-1 text-xs font-semibold border rounded-lg mr-1" data-testid={`trade-declare-${r.file_number}`}>Beyan</button>}
                  {canEdit && r.status === "declared" && <button onClick={() => setStatus(r, "cleared")} className="px-2 py-1 text-xs font-semibold border rounded-lg mr-1" data-testid={`trade-clear-${r.file_number}`}>Gümrük çıktı</button>}
                  {canEdit && !r.invoice_id && r.status !== "cancelled" && <button onClick={() => convert(r)} className="px-2 py-1 text-xs font-semibold bg-emerald-600 text-white rounded-lg mr-1" data-testid={`trade-invoice-${r.file_number}`}><FileText className="w-3 h-3 inline mr-1" />Fatura</button>}
                  <button onClick={() => setPrintDoc({ ...r, doc: "commercial" })} className="px-2 py-1 text-xs font-semibold border rounded-lg mr-1" data-testid={`trade-print-${r.file_number}`}>Yazdır</button>
                  {canEdit && !r.invoice_id && r.status !== "cancelled" && <button onClick={() => remove(r)} className="p-1 text-rose-500" data-testid={`trade-del-${r.file_number}`}><Trash2 className="w-4 h-4" /></button>}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 z-[70] bg-slate-900/60 flex items-start justify-center p-4 overflow-y-auto" onClick={() => setForm(null)}>
          <div className="bg-white rounded-2xl w-full max-w-5xl my-6 p-5 space-y-4 text-xs shadow-2xl" onClick={(e) => e.stopPropagation()} data-testid="trade-modal">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div><label className="font-semibold block mb-1">Durum</label><select value={form.status || "draft"} onChange={(e) => setF("status", e.target.value)} className="w-full border rounded-lg p-2 bg-slate-50" data-testid="trade-status">{(meta?.statuses || [["draft", "Taslak"]]).map(([c, l]) => <option key={c} value={c}>{l}</option>)}</select></div>
            </div>

            {form.kind === "import" && (
              <div className="grid grid-cols-2 sm:grid-cols-7 gap-2 bg-amber-50 border border-amber-100 rounded-xl p-3" data-testid="trade-import-costs">
                ...
                <div><label className="font-semibold block mb-1">Damga ₺</label><input type="number" value={form.stamp_tax} onChange={(e) => setF("stamp_tax", e.target.value)} className="w-full border rounded-lg p-2" data-testid="trade-stamp" /></div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              {form.items.map((it, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center" data-testid={`trade-item-${i}`}>
                  <div className="col-span-3"><SearchSelect value={it.product_id} options={products} getLabel={(p) => p.name} getSub={(p) => `${p.sku || ""} ${p.gtip ? "GTIP " + p.gtip : ""}`} placeholder="Ürün" onChange={(id) => pickProduct(i, id)} testId={`trade-item-prod-${i}`} /></div>
                  ...
                  <input value={it.origin_country || ""} onChange={(e) => setItem(i, "origin_country", e.target.value)} placeholder="Menşe" className="col-span-1 border rounded-lg p-2" data-testid={`trade-item-origin-${i}`} />
                  <input type="number" value={it.net_weight || ""} onChange={(e) => setItem(i, "net_weight", e.target.value)} placeholder="Kg" className="col-span-1 border rounded-lg p-2" data-testid={`trade-item-weight-${i}`} />
                  ...
                  <input type="number" step="0.01" value={it.unit_price_fx} onChange={(e) => setItem(i, "unit_price_fx", e.target.value)} className="col-span-1 border rounded-lg p-2 text-right" data-testid={`trade-item-price-${i}`} />
                  <div className="col-span-1 text-right font-semibold">{fmtMoney((Number(it.quantity) || 0) * (Number(it.unit_price_fx) || 0), form.currency)}</div>
        ...
        <div className="fixed inset-0 z-[80] bg-slate-900/60 flex items-start justify-center p-4 overflow-y-auto" onClick={() => setPrintDoc(null)}>
          <div className="bg-white rounded-2xl w-full max-w-3xl my-6 p-6 text-xs shadow-2xl print:shadow-none print:my-0" onClick={(e) => e.stopPropagation()} data-testid="trade-print-modal">
            <div className="flex justify-between items-center mb-4 print:hidden">
              <div className="flex gap-2">
                <button onClick={() => setPrintDoc({ ...printDoc, doc: "commercial" })} className={`px-3 py-1.5 rounded-lg font-semibold ${printDoc.doc === "commercial" ? "bg-slate-900 text-white" : "border"}`}>Ticari fatura</button>
                <button onClick={() => setPrintDoc({ ...printDoc, doc: "packing" })} className={`px-3 py-1.5 rounded-lg font-semibold ${printDoc.doc === "packing" ? "bg-slate-900 text-white" : "border"}`}>Packing list</button>
              </div>
              <div className="flex gap-2">
                <button onClick={() => window.print()} className="px-3 py-1.5 bg-sky-600 text-white rounded-lg font-bold">Yazdır</button>
                <button onClick={() => setPrintDoc(null)} className="px-3 py-1.5 border rounded-lg">Kapat</button>
              </div>
            </div>
            <div className="uppercase text-[10px] tracking-widest text-slate-400 font-bold">{printDoc.doc === "packing" ? "Packing List" : "Commercial Invoice"}</div>
            <div className="text-lg font-bold mt-1">{printDoc.file_number} · {printDoc.kind === "import" ? "İTHALAT" : "İHRACAT"}</div>
            <div className="grid grid-cols-2 gap-4 mt-3">
              <div><div className="text-slate-400">Cari</div><div className="font-bold">{printDoc.contact_name}</div></div>
              <div><div className="text-slate-400">Teslim / Ülke / Rejim</div><div className="font-semibold">{printDoc.incoterm} · {printDoc.country || "—"} · {printDoc.regime_code}</div></div>
              <div><div className="text-slate-400">Gümrük / Beyanname</div><div>{printDoc.customs_office || "—"} {printDoc.declaration_no ? `· ${printDoc.declaration_no}` : ""}</div></div>
              <div><div className="text-slate-400">BL / Konteyner / DAB</div><div>{[printDoc.bl_awb, printDoc.container_no, printDoc.dab_no].filter(Boolean).join(" · ") || "—"}</div></div>
            </div>
            <table className="w-full mt-4 text-left">
              <thead className="border-b text-slate-500 uppercase text-[10px]"><tr>
                <th className="py-1">Kalem</th><th>GTIP</th><th>Menşe</th><th className="text-right">Miktar</th>
                {printDoc.doc === "packing" ? <th className="text-right">Kg</th> : <th className="text-right">Birim</th>}
                {printDoc.doc !== "packing" && <th className="text-right">Tutar</th>}
              </tr></thead>
              <tbody>
                {(printDoc.items || []).map((it, i) => (
                  <tr key={i} className="border-b border-slate-100">
                    <td className="py-1.5 font-semibold">{it.name}</td>
                    <td className="font-mono">{it.gtip || "—"}</td>
                    <td>{it.origin_country || printDoc.country || "—"}</td>
                    <td className="text-right">{it.quantity} {it.unit || ""}</td>
                    {printDoc.doc === "packing" ? <td className="text-right">{it.net_weight || "—"}</td> : <td className="text-right">{fmtMoney(it.unit_price_fx, printDoc.currency)}</td>}
                    {printDoc.doc !== "packing" && <td className="text-right font-bold">{fmtMoney((Number(it.quantity) || 0) * (Number(it.unit_price_fx) || 0), printDoc.currency)}</td>}
                  </tr>
                ))}
              </tbody>
            </table>
            {printDoc.doc !== "packing" && (
              <div className="text-right mt-3 font-bold">Toplam {fmtMoney(printDoc.amount_fx, printDoc.currency)} · ₺ {fmtMoney(printDoc.amount_try, "TRY")}</div>
            )}
          </div>
        </div>
      )}
