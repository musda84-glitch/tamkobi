...
import { Building2, MessageSquare, Mail, Landmark, ShoppingCart, Truck, FileCheck2, Printer, Upload, Save, Loader2, ListOrdered, Link as LinkIcon, Ruler, Trash2, Pencil, Users, ShieldCheck, Coins, HardDrive } from "lucide-react";
...
export const B2BSettings = ({ companyId }) => {
  ...
  const toggleCustomer = async (c, enabled) => {
    try {
      const r = await axios.post(`${API_URL}/contacts/${c.id}/b2b-access`, { enabled, base_url: window.location.origin, discount: c.b2b_discount || st.default_discount });
      if (enabled) {
        ...
        toast.success(loopback ? "Portal linki kopyalandı (yalnızca bu bilgisayarda açılır — müşteri için canlı adres gerekir)." : "Portal linki kopyalandı.");
      } else toast.success("B2B erişimi kapatıldı.");
      load();
  ...
  return (
    <div className="space-y-4 text-xs" data-testid="b2b-settings">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
        <div className="flex items-center justify-between"><div><div className="text-sm font-bold text-slate-900">B2B Portal Özellikleri</div><div className="text-slate-500 flex flex-wrap items-center gap-1.5">Müşteri giriş adresi: <b className="font-mono text-emerald-700" data-testid="b2b-login-url">{window.location.origin}/b2b/giris</b><button type="button" onClick={() => { navigator.clipboard?.writeText(`${window.location.origin}/b2b/giris`); toast.success("Kopyalandı"); }} className="px-1.5 py-0.5 border rounded text-[10px] font-semibold hover:bg-slate-50" data-testid="b2b-copy-login-url">kopyala</button><span>· e-posta/VKN + şifre ya da /portal/… tokenli link; {d.active_count} müşteri aktif.</span></div></div><span className={`px-2 py-1 rounded-lg font-bold ${st.enabled ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{st.enabled ? "AÇIK" : "KAPALI"}</span></div>
        {["localhost", "127.0.0.1"].includes(window.location.hostname) && (
          <div className="text-[11px] text-amber-800 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2" data-testid="b2b-loopback-hint">
            Yerel adres (<code className="font-mono">127.0.0.1</code>) yalnızca bu bilgisayarda açılır. Müşteriye link verirken Platform Ayarları’ndaki <b>canlı site adresini</b> kullanın; aksi halde telefonda Error Code -102 (bağlantı reddedildi) görünür. Token’sız <code className="font-mono">/portal</code> yerine <code className="font-mono">/b2b/giris</code> kullanın.
          </div>
        )}
