  ...
  return (
    <div className="space-y-4 text-xs" data-testid="b2b-settings">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
        <div className="flex items-center justify-between"><div><div className="text-sm font-bold text-slate-900">Şirket Varsayılanları (B2B)</div><div className="text-slate-500 flex flex-wrap items-center gap-1.5">Yeni cariler ve özel ayarı olmayan cariler bu değerleri kullanır. Cari kartı → <b>B2B Portal</b> ile müşteri bazında ezilir. Giriş: <b className="font-mono text-emerald-700" data-testid="b2b-login-url">{window.location.origin}/b2b/giris</b><button type="button" onClick={() => { navigator.clipboard?.writeText(`${window.location.origin}/b2b/giris`); toast.success("Kopyalandı"); }} className="px-1.5 py-0.5 border rounded text-[10px] font-semibold hover:bg-slate-50" data-testid="b2b-copy-login-url">kopyala</button><span>· {d.active_count} müşteri aktif.</span></div></div><span className={`px-2 py-1 rounded-lg font-bold ${st.enabled ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{st.enabled ? "AÇIK" : "KAPALI"}</span></div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
        <div className="flex items-center justify-between gap-2"><div><div className="text-sm font-bold text-slate-900">Müşteri Erişimleri</div><div className="text-slate-500 text-[11px]">Hızlı erişim/şifre. Özellik detayı için cari kartını açıp <b>B2B Portal</b> sekmesine gidin.</div></div><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Müşteri ara…" className="bg-slate-50 border rounded-lg p-2 w-56" data-testid="b2b-customer-search" /></div>
