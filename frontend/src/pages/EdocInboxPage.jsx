...
import { Inbox, Upload, CheckCircle2, XCircle, UserPlus, PackagePlus, Loader2, Link2 } from "lucide-react";
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3" data-testid="edoc-detail">
          {!sel ? <div className="text-slate-400 p-6 text-center">Detay için soldan belge seçin.</div> : (<>
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div><div className="text-sm font-bold text-slate-900">{sel.kind === "dispatch" ? "Gelen e-İrsaliye" : "Gelen e-Fatura"} {sel.number}</div><div className="text-slate-500">{sel.issue_date} · {sel.source === "ubl_xml" ? "UBL XML" : "PDF (AI)"} · {sel.profile || ""} {sel.type_code || ""}</div></div>
