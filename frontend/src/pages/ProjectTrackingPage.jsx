    ...
    <div className="min-h-screen bg-slate-100 py-6 px-3 sm:px-6" data-testid="public-project-page">
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-8 space-y-5">
          ...
          {p.statement && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-2" data-testid="public-project-statement">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a href={`${API_URL}/public/projects/${token}/statement.pdf`} className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-700 bg-white border border-indigo-200 px-2 py-1 rounded-lg" data-testid="public-project-statement-pdf"><FileDown className="w-3.5 h-3.5" /> PDF</a>
                </div>
              </div>
              <div className="overflow-x-auto bg-white rounded-lg border border-slate-100">
                <table className="w-full text-left text-xs">
                  <thead className="text-[10px] uppercase text-slate-400"><tr><th className="py-1.5 px-2">Tarih</th><th className="py-1.5 px-2">Belge</th><th className="py-1.5 px-2 text-right">Borç</th><th className="py-1.5 px-2 text-right">Alacak</th><th className="py-1.5 px-2 text-right">Bakiye</th></tr></thead>
                  <tbody>
                    ...
                    {(p.statement.rows || []).map((r, i) => (
                      <tr key={`${r.date}-${i}`} className="border-t border-slate-100" data-testid={`public-project-statement-row-${i}`}>
                        ...
                        <td className="py-1.5 px-2 text-right font-semibold">{fmt(r.balance)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
