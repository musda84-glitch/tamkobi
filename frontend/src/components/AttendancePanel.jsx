...
export const AttendancePanel = ({ companyId }) => {
  ...
  return (
    <div className="space-y-4 text-xs" data-testid="attendance-panel">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden"><div className="px-4 py-2.5 border-b font-bold text-slate-900 flex items-center justify-between"><span>Günlük Kayıtlar ({data.records.length})</span><span className="text-[10px] text-slate-400 font-normal">Personel kendi kaydını Mesaim ekranından onaylar veya itiraz eder</span></div><div className="max-h-72 overflow-y-auto divide-y divide-slate-100">{data.records.map((r) => (
      ...
      {schedEmp && <EmployeeScheduleModal employee={schedEmp} companySchedule={data.schedule} labels={DAY_LABELS} onClose={() => setSchedEmp(null)} onSaved={load} />}
    </div>
  );
};
