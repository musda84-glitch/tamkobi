...
export const ProductionOrderModal = ({ companyId, product, recipes: recipesProp, onClose, onCreated }) => {
  ...
  const [qty, setQty] = useState(product?._planQty || (product ? Math.max(1, Math.ceil((product.min_stock_alert || 0) - (product.stock_quantity || 0))) || 1 : 1));
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const [notes, setNotes] = useState(product?._planNotes || "");
