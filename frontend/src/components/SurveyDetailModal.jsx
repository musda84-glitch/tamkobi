...
export const SurveyDetailModal = ({ survey, onClose, onChanged }) => {
  ...
  const upload = async (e) => {
    ...
    setUploading(true);
    try {
      const r = await axios.post(`${API_URL}/files/upload?entity=survey&entity_id=${surveyId}&company_id=${survey.company_id || ""}`, fd);
  ...
  const save = async () => {
    try {
      await axios.put(`${API_URL}/surveys/${surveyId}`, { notes, status });
      toast.success("Keşif güncellendi.");
      if (status === "done" && !survey.quote_id && window.confirm("Keşif tamamlandı. Teklife dönüştürülsün mü? (İptal = sadece durum; teklifi sonra ayrı oluşturabilirsiniz.)")) {
        await axios.post(`${API_URL}/surveys/${surveyId}/convert-to-quote`);
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-900/50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl max-w-2xl w-full p-4 sm:p-5 space-y-3 text-xs shadow-2xl max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} data-testid="survey-detail-modal">
        ...
        <div>
          <div className="text-[10px] uppercase text-slate-400 font-semibold mb-1">Fotoğraflar</div>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {images.map((img) => <a key={img} href={resolveImageUrl(img)} target="_blank" rel="noreferrer"><img src={resolveImageUrl(img)} alt="" className="w-full h-20 object-cover rounded-lg border" /></a>)}
            <label className="h-20 rounded-lg border-2 border-dashed border-slate-300 hover:border-emerald-500 flex flex-col items-center justify-center cursor-pointer text-slate-400 text-[10px] gap-1" data-testid="survey-upload-photo">
              <ImagePlus className="w-4 h-4" />{uploading ? "Yükleniyor…" : "Ekle"}
              <input type="file" accept="image/*,.jpg,.jpeg,.png,.webp,.gif" capture="environment" className="hidden" onChange={upload} disabled={uploading} />
            </label>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div className="sm:col-span-2"><label className="block font-semibold mb-1">Notlar</label><textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="w-full bg-slate-50 border rounded-lg p-2" data-testid="survey-notes-input" /></div>
