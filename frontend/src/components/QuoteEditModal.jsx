...
export const QuoteEditModal = ({ quote, onClose, onSaved }) => {
  ...
  const ccy = quote.currency || "TRY";
  const upd = (i, k, v) => setItems(items.map((x, idx) => idx === i ? { ...x, [k]: v } : x));
  const shownLine = (i, field, stored) => lineNumberShown(lineDrafts, lineDraftKey(i, field), stored);
  const focusLine = (i, field) => setLineDrafts((m) => ({ ...m, [lineDraftKey(i, field)]: lineNumberOnFocus() }));
  const changeLine = (i, field, raw, emptyFallback) => {
    setLineDrafts((m) => ({ ...m, [lineDraftKey(i, field)]: raw }));
    const next = lineNumberCommit(raw, emptyFallback);
    if (next != null) upd(i, field, next);
  };
  const blurLine = (i, field) => {
    setLineDrafts((m) => {
      const copy = { ...m };
      delete copy[lineDraftKey(i, field)];
      return copy;
    });
  };
      ...
      <div className="bg-white rounded-2xl max-w-3xl w-full p-5 space-y-3 text-xs shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} data-testid="quote-edit-modal">
        ...
        <table className="w-full"><thead className="text-slate-500 uppercase text-[10px] border-b"><tr><th className="text-left py-1">Kalem</th><th className="py-1 w-20">Miktar</th><th className="py-1 w-16">Birim</th><th className="py-1 w-28">Birim Fiyat</th><th className="py-1 w-16">KDV</th><th className="py-1 text-right">Tutar</th><th className="w-8"></th></tr></thead>
          <tbody className="divide-y divide-slate-100">{items.map((it, i) => (
            <tr key={i}><td className="py-1"><input value={it.name} onChange={(e) => upd(i, "name", e.target.value)} className="w-full bg-slate-50 border rounded p-1" data-testid={`quote-item-name-${i}`} /></td><td className="py-1"><input type="text" inputMode="decimal" value={shownLine(i, "quantity", it.quantity)} onFocus={() => focusLine(i, "quantity")} onBlur={() => blurLine(i, "quantity")} onChange={(e) => changeLine(i, "quantity", e.target.value, 1)} className="w-full bg-slate-50 border rounded p-1 text-center" data-testid={`quote-item-qty-${i}`} /></td><td className="py-1"><input value={it.unit || "Adet"} onChange={(e) => upd(i, "unit", e.target.value)} className="w-full bg-slate-50 border rounded p-1 text-center" /></td><td className="py-1"><input type="text" inputMode="decimal" value={shownLine(i, "unit_price", it.unit_price)} onFocus={() => focusLine(i, "unit_price")} onBlur={() => blurLine(i, "unit_price")} onChange={(e) => changeLine(i, "unit_price", e.target.value, 0)} className="w-full bg-slate-50 border rounded p-1 text-right" data-testid={`quote-item-price-${i}`} /></td><td className="py-1"><select value={it.vat_rate ?? 20} onChange={(e) => upd(i, "vat_rate", Number(e.target.value))} className="w-full bg-slate-50 border rounded p-1">{[20, 10, 1, 0].map((v) => <option key={v} value={v}>%{v}</option>)}</select></td><td className="py-1 text-right font-bold">{fmt(Number(it.quantity || 0) * Number(it.unit_price || 0), ccy)}</td><td className="text-right"><button onClick={() => setItems(items.filter((_, idx) => idx !== i))} className="text-rose-500" title="Sil"><Trash2 className="w-3.5 h-3.5" /></button></td></tr>
          ))}</tbody></table>
        ...
        <div className="flex justify-between items-center border-t pt-2"><span className="font-bold">Genel Toplam (KDV dahil): {fmt(total, ccy)}</span><div className="flex gap-2"><button onClick={onClose} className="px-3 py-1.5 border rounded-lg">İptal</button><button onClick={save} className="px-4 py-1.5 bg-emerald-600 text-white rounded-lg font-semibold" data-testid="quote-edit-save">Kaydet</button></div></div>
      </div>
    </div>
  );
};
