...
import { Mail, Server, Plus, Trash2, Loader2, Save, Shield, Send, PlugZap, Pencil, X } from "lucide-react";
import { API_URL } from "../../context/AuthContext";
import { inputCls, Toggle } from "./saasUi";

const cred = { withCredentials: true };

const emptyServer = () => ({ name: "", provider: "custom", smtp_host: "", smtp_port: 587, smtp_user: "", password: "" });
const emptyBox = (serverId = "") => ({
  email: "",
  display_name: "",
  server_id: serverId,
...
export const PlatformMailPanel = () => {
  ...
  const [srv, setSrv] = useState(emptyServer());
  ...
  const resetServerForm = (keepProvider = true) => {
    setEditingServerId(null);
    setSrv({
      ...emptyServer(),
      provider: keepProvider ? srv.provider : "custom",
      smtp_host: keepProvider ? srv.smtp_host : "",
      smtp_port: keepProvider ? srv.smtp_port : 587,
    });
  };

  const resetBoxForm = () => {
    setEditingBoxId(null);
    setBox(emptyBox(d.servers[0]?.id || ""));
  };
  ...
  const startEditServer = (s) => {
    ...
    document.querySelector('[data-testid="mail-server-form"]')?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  const saveServer = async (e) => {
    ...
    try {
      if (editingServerId) {
        ...
        await axios.put(`${API_URL}/system/mail/servers/${editingServerId}`, payload, cred);
  ...
  const testServer = async (id) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || "Test edilemedi.");
    } finally {
      setBusy("");
    }
  };
  ...
  const delServer = async (s) => {
    ...
    try {
      await axios.delete(`${API_URL}/system/mail/servers/${s.id}`, cred);
      toast.success("Sunucu silindi.");
      if (editingServerId === s.id) resetServerForm(false);
      load();
    } catch (err) {
  ...
  const startEditBox = (b) => {
    setEditingBoxId(b.id);
    setBox({
      email: b.email || "",
      display_name: b.display_name || "",
      server_id: b.server_id || d.servers[0]?.id || "",
      purposes: Array.isArray(b.purposes) && b.purposes.length ? [...b.purposes] : ["general"],
      allow_all_admins: b.allow_all_admins !== false,
      allowed_user_ids: Array.isArray(b.allowed_user_ids) ? [...b.allowed_user_ids] : [],
      is_default: !!b.is_default,
      password: "",
      smtp_user: b.smtp_user || "",
    });
    document.querySelector('[data-testid="mail-box-form"]')?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  const saveBox = async (e) => {
    ...
    try {
      if (editingBoxId) {
        const payload = { ...box };
        if (!(payload.password || "").trim()) delete payload.password;
        await axios.put(`${API_URL}/system/mail/mailboxes/${editingBoxId}`, payload, cred);
  ...
  const patchBox = async (b, body, ok) => {
    try {
      await axios.put(`${API_URL}/system/mail/mailboxes/${b.id}`, body, cred);
      toast.success(ok);
      load();
    } catch (err) {
  ...
  const delBox = async (b) => {
    ...
    try {
      await axios.delete(`${API_URL}/system/mail/mailboxes/${b.id}`, cred);
      toast.success("Kutu silindi.");
      if (editingBoxId === b.id) resetBoxForm();
      load();
    } catch (err) {
      toast.error(err.response?.data?.detail || "Silinemedi.");
    }
      ...
      <form onSubmit={saveServer} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3" data-testid="mail-server-form">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
            ...
            {editingServerId ? "SMTP sunucusunu düzenle" : "SMTP sunucusu"}
          </h3>
          {editingServerId && (
            <button type="button" onClick={() => resetServerForm(false)} className="text-[11px] font-semibold text-slate-500 inline-flex items-center gap-1" data-testid="mail-srv-cancel-edit">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Şifre / uygulama şifresi</label>
            <input type="password" required={!editingServerId} value={srv.password} onChange={(e) => setSrv({ ...srv, password: e.target.value })} placeholder={editingServerId ? "Boş bırakırsanız değişmez" : ""} className={inputCls} data-testid="mail-srv-password" />
          </div>
        </div>
        <div className="flex justify-end gap-2">
          <button type="submit" disabled={busy === "srv"} className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold inline-flex items-center gap-1.5 disabled:opacity-60" data-testid="mail-srv-save">
            {busy === "srv" ? <Loader2 className="w-4 h-4 animate-spin" /> : editingServerId ? <Save className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
            {editingServerId ? "Sunucuyu kaydet" : "Sunucu ekle"}
          </button>
        </div>
      </form>
        ...
        {d.servers.length === 0 ? <div className="p-6 text-slate-400">Henüz SMTP sunucusu yok.</div> : d.servers.map((s) => (
          <div key={s.id} className="px-4 py-3 border-t border-slate-100 flex flex-wrap items-center gap-3" data-testid={`mail-srv-row-${s.id}`}>
            <div className="flex-1 min-w-[200px]">
              ...
              <div className="text-[10px] font-mono text-slate-500">{s.smtp_host}:{s.smtp_port} · {s.smtp_user || "kullanıcı yok"}</div>
              {s.last_test && <div className={`text-[10px] mt-0.5 ${s.last_test.ok ? "text-emerald-700" : "text-rose-600"}`}>Son test: {s.last_test.ok ? "başarılı" : s.last_test.message}</div>}
            </div>
            <button type="button" onClick={() => startEditServer(s)} className="px-3 py-1.5 border border-slate-300 text-slate-700 rounded-lg font-semibold inline-flex items-center gap-1 hover:bg-slate-50" data-testid={`mail-srv-edit-${s.id}`}>
              <Pencil className="w-3.5 h-3.5" /> Düzenle
            </button>
            <button type="button" onClick={() => testServer(s.id)} disabled={busy === `t-${s.id}`} className="px-3 py-1.5 border border-sky-300 text-sky-700 rounded-lg font-semibold inline-flex items-center gap-1" data-testid={`mail-srv-test-${s.id}`}>
              {busy === `t-${s.id}` ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <PlugZap className="w-3.5 h-3.5" />} Test
            </button>
      ...
      <form onSubmit={saveBox} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3" data-testid="mail-box-form">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
            ...
            {editingBoxId ? "Gönderen kimliğini düzenle" : "Bildiğim mailler (gönderen kimlikleri)"}
          </h3>
          {editingBoxId && (
            <button type="button" onClick={resetBoxForm} className="text-[11px] font-semibold text-slate-500 inline-flex items-center gap-1" data-testid="mail-box-cancel-edit">
              <X className="w-3.5 h-3.5" /> Vazgeç
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Kutu şifresi (boş = sunucu şifresi)</label>
            <input type="password" value={box.password} onChange={(e) => setBox({ ...box, password: e.target.value })} placeholder={editingBoxId ? "Boş bırakırsanız değişmez" : ""} className={inputCls} data-testid="mail-box-password" />
          </div>
        </div>
        <div>
        ...
        <label className="flex items-center gap-2 font-semibold text-slate-700">
          <Toggle on={box.allow_all_admins} onChange={(v) => setBox({ ...box, allow_all_admins: v, allowed_user_ids: v ? [] : box.allowed_user_ids })} testId="mail-box-allow-all" />
          Tüm panel yöneticileri kullanabilir
        </label>
          ...
          <div className="bg-slate-50 rounded-xl p-3 space-y-1.5" data-testid="mail-box-acl">
            ...
            {d.admins.map((a) => (
              <label key={a.id} className="flex items-center gap-2">
                <input type="checkbox" checked={box.allowed_user_ids.includes(a.id)} onChange={() => toggleUser(a.id)} data-testid={`mail-box-acl-${a.id}`} />
        ...
        <label className="flex items-center gap-2 font-semibold text-slate-700">
          <Toggle on={box.is_default} onChange={(v) => setBox({ ...box, is_default: v })} testId="mail-box-default" />
          Sistem işleri için varsayılan (hatırlatma / fatura)
        </label>
        <div className="flex justify-end">
          <button type="submit" disabled={busy === "box" || !d.servers.length} className="px-4 py-2 bg-emerald-600 text-white rounded-xl font-bold inline-flex items-center gap-1.5 disabled:opacity-60" data-testid="mail-box-save">
            {busy === "box" ? <Loader2 className="w-4 h-4 animate-spin" /> : editingBoxId ? <Save className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
            {editingBoxId ? "Kutuyu kaydet" : "Kutu ekle"}
          </button>
        </div>
      </form>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-x-auto" data-testid="mail-box-list">
        <table className="w-full min-w-[720px]">
          <thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500">
            <tr>
              <th className="px-3 py-2.5 text-left">Kutu</th>
              <th className="px-3 py-2.5 text-left">Sunucu</th>
              <th className="px-3 py-2.5 text-left">Kim kullanır</th>
            ...
            {d.mailboxes.map((b) => (
              <tr key={b.id} data-testid={`mail-box-row-${b.id}`}>
                <td className="px-3 py-2">
                  <div className="font-semibold text-slate-800">{b.display_name}</div>
                  <div className="font-mono text-[10px] text-slate-500">{b.email}</div>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {(b.purposes || []).map((p) => <span key={p} className="text-[9px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">{(d.purposes.find((x) => x.id === p) || {}).label || p}</span>)}
                    {b.is_default && <span className="text-[9px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-bold">varsayılan</span>}
                  </div>
                </td>
                ...
                <td className="px-3 py-2">
                  <label className="flex items-center gap-2">
                    <Toggle on={!!b.is_active} onChange={(v) => patchBox(b, { is_active: v }, v ? "Aktif." : "Pasif.")} testId={`mail-box-active-${b.id}`} />
                ...
                <td className="px-3 py-2 text-right whitespace-nowrap">
                  ...
                  {b.is_default && <span className="text-[10px] font-semibold text-amber-700 mr-2">Varsayılan</span>}
                  <button type="button" onClick={() => startEditBox(b)} className="p-1.5 text-slate-600 hover:text-slate-900" title="Düzenle" data-testid={`mail-box-edit-${b.id}`}><Pencil className="w-4 h-4" /></button>
