    ...
    <form onSubmit={save} className="space-y-4 text-xs max-w-3xl" data-testid="saas-settings">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div><label className="block font-semibold text-slate-700 mb-1">Uygulama adresi (yenileme linkleri için)</label><input value={s.public_url || ""} onChange={(e) => setS({ ...s, public_url: e.target.value })} placeholder={window.location.origin} className={inputCls} data-testid="set-public-url" /></div>
