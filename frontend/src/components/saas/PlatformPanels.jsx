    ...
    <form onSubmit={save} className="space-y-4" data-testid="saas-settings">
      <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div><label className="block font-semibold text-slate-700 mb-1">Gönderici hesapların alındığı şirket</label><select value={s.sender_company_id} onChange={(e) => setS({ ...s, sender_company_id: e.target.value })} className={inputCls} data-testid="set-sender-company">{s.companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select><div className="text-[10px] text-slate-500 mt-1">Yedek SMTP (şirket): {s.sender_mail || "tanımlı değil"} · Platform posta: {s.platform_mail_from || "tanımlı değil"} · WhatsApp: {s.sender_whatsapp_ready ? "hazır" : "tanımlı değil (simüle)"}</div><a href="/sistem/posta" className="inline-block mt-1 text-[11px] font-semibold text-emerald-700" data-testid="set-go-mail">Posta sunucusu ve yetkiler →</a></div>
