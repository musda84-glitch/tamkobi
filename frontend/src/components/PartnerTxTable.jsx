...
export const PartnerTxTable = ({ txs, accounts, onChanged }) => {
      ...
      <tbody className="divide-y divide-slate-100">
        ...
        {rows.map((t) => edit?.id === t.id ? (
          <tr key={t.id} className="bg-indigo-50/40" data-testid={`partner-tx-edit-row-${t.id}`}>
            ...
            <td className="px-4 py-2 flex gap-1"><select value={edit.account_id || ""} onChange={(e) => setEdit({ ...edit, account_id: e.target.value })} className={inputCls} data-testid="partner-tx-edit-account">{accounts.map((a) => <option key={a.id} value={a.id}>{a.account_name}</option>)}</select><input value={edit.description || ""} onChange={(e) => setEdit({ ...edit, description: e.target.value })} className={`${inputCls} flex-1`} placeholder="Açıklama" data-testid="partner-tx-edit-desc" /></td>
