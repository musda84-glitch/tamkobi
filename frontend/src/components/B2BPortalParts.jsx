...
import { ShoppingCart, Truck, Trash2, Building2, X, ExternalLink, PackageCheck, Clock, Pencil, Ban, Plus, Minus, Loader2 } from "lucide-react";
...
export const OrdersList = ({ orders, products, company }) => {
  ...
  <div className="bg-white rounded-2xl border overflow-hidden" data-testid="b2b-orders">
    ...
    <div className="md:hidden divide-y">{orders.map((o) => (
      <div key={o.id} className="p-3 text-xs space-y-1" data-testid={`b2b-order-${o.order_number}`}>
        ...
        <div className="flex items-center justify-between gap-2"><span className="font-bold text-sm">{fmt(o.total_amount)} ₺</span><PreviewOrderBtn onClick={() => setPreview(o)} orderNumber={o.order_number} /></div>
        <TrackingCard t={o.tracking} orderNumber={o.order_number} />
      </div>))}</div>
    {orders.length > 0 && <table className="hidden md:table w-full text-xs"><thead className="bg-slate-50 text-slate-500 uppercase text-[10px] border-b"><tr><Th>Sipariş</Th><Th>Sizin no</Th><Th>Tarih</Th><Th>Kalem</Th><Th right>Tutar</Th><Th>Durum</Th><Th>Kargo</Th><Th>İşlem</Th></tr></thead><tbody className="divide-y">{orders.map((o) => <tr key={o.id}><td className="p-3 font-mono font-bold">{o.order_number}</td><td className="p-3 font-mono text-slate-600">{o.customer_order_number || "—"}</td><td className="p-3 text-slate-500">{(o.order_date || "").slice(0, 10)}</td><td className="p-3">{(o.items || []).map((i) => formatOrderItemLabel(i)).join(", ")}</td><td className="p-3 text-right font-bold">{fmt(o.total_amount)} ₺</td><td className="p-3"><span className="bg-slate-100 px-1.5 py-0.5 rounded font-semibold">{statusTr(o.order_status)}</span></td><td className="p-3 min-w-[260px]"><TrackingCard t={o.tracking} orderNumber={o.order_number} /></td><td className="p-3"><PreviewOrderBtn onClick={() => setPreview(o)} orderNumber={o.order_number} /></td></tr>)}</tbody></table>}
    ...
    {orders.length > 0 && <table className="hidden md:table w-full text-xs"><thead className="bg-slate-50 text-slate-500 uppercase text-[10px] border-b"><tr><Th>Sipariş</Th><Th>Tarih</Th><Th>Kalem</Th><Th right>Tutar</Th><Th>Durum</Th><Th>Kargo</Th></tr></thead><tbody className="divide-y">{orders.map((o) => <tr key={o.id}><td className="p-3 font-mono font-bold">{o.order_number}</td><td className="p-3 text-slate-500">{(o.order_date || "").slice(0, 10)}</td><td className="p-3">{(o.items || []).map((i) => `${i.quantity}× ${i.product_name}`).join(", ")}</td><td className="p-3 text-right font-bold">{fmt(b2bOrderGross(o))} ₺</td><td className="p-3"><span className="bg-slate-100 px-1.5 py-0.5 rounded font-semibold">{statusTr(o.order_status)}</span></td><td className="p-3 min-w-[260px]"><TrackingCard t={o.tracking} orderNumber={o.order_number} /></td></tr>)}</tbody></table>}
...
export const OrdersList = ({ orders, token, products, onChanged }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border overflow-hidden" data-testid="b2b-orders">
      ...
      {orders.length > 0 && <table className="hidden md:table w-full text-xs"><thead className="bg-slate-50 text-slate-500 uppercase text-[10px] border-b"><tr><Th>Sipariş</Th><Th>Tarih</Th><Th>Kalem</Th><Th right>Tutar</Th><Th>Durum</Th><Th>Kargo</Th><Th>İşlem</Th></tr></thead><tbody className="divide-y">{orders.map((o) => <tr key={o.id} data-testid={`b2b-order-row-${o.order_number}`}><td className="p-3 font-mono font-bold">{o.order_number}</td><td className="p-3 text-slate-500">{(o.order_date || "").slice(0, 10)}</td><td className="p-3">{(o.items || []).map((i) => `${i.quantity}× ${i.product_name}`).join(", ")}</td><td className="p-3 text-right font-bold">{fmt(o.total_amount)} ₺</td><td className="p-3"><OrderStatusBadge o={o} /></td><td className="p-3 min-w-[220px]"><TrackingCard t={o.tracking} orderNumber={o.order_number} /></td><td className="p-3"><OrderActions o={o} onEdit={setEdit} onDelete={remove} onCancel={setCancel} busy={busyId === o.id} /></td></tr>)}</tbody></table>}
      {edit && <EditOrderModal order={edit} products={products} token={token} onClose={() => setEdit(null)} onDone={onChanged} />}
      {cancel && <CancelRequestModal order={cancel} token={token} onClose={() => setCancel(null)} onDone={onChanged} />}
    </div>
  );
