...
export const GlobalSearch = ({ companyId, className = "" }) => {
  ...
  return (
    <div className={`relative min-w-0 flex-1 max-w-xl ${className}`} ref={box} data-testid="global-search">
      ...
      <input value={q} onChange={(e) => setQ(e.target.value)} onFocus={() => res && setOpen(true)} placeholder="Ara: cari, ürün, sipariş, fatura… (Ctrl+K)" className="w-full bg-slate-100 border border-transparent focus:border-slate-300 focus:bg-white rounded-lg pl-8 pr-2 py-1.5 text-xs focus:outline-none transition-all" data-testid="global-search-input" />
      {open && res && (
        <div className="absolute left-0 top-full mt-1.5 w-[min(420px,90vw)] bg-white border border-slate-200 rounded-xl shadow-2xl z-50 max-h-[70vh] overflow-y-auto p-1.5 text-xs" data-testid="global-search-results">
          ...
          {groups.map(([title, Icon, items, fmt]) => (
            <div key={title} className="mb-1">
              ...
              {items.map((it) => { const [l, sub, path] = fmt(it); return <button key={it.id} type="button" onClick={() => go(path)} className="w-full text-left px-2.5 py-1.5 rounded-lg hover:bg-slate-50 flex items-center justify-between gap-2" data-testid={`gs-result-${it.id}`}><span className="font-semibold text-slate-800 truncate">{l}</span><span className="text-[10px] text-slate-400 truncate">{sub}</span></button>; })}
            </div>))}
        </div>
      )}
    </div>
