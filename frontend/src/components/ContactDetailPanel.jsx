...
<button type="button" onClick={(e) => openInvCtxFromButton(e, inv)} className="p-1.5 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition" title={incoming ? "Gelen e-fatura işlemleri" : "Fatura kesim & diğer işlemler"} data-testid={`detail-inv-more-${inv.invoice_number}`}><MoreVertical className="w-4 h-4" /></button>
          ...
          {tab === "projects" && (
            <table className="w-full text-left" data-testid="detail-projects-list">
              <thead className="text-slate-500 uppercase text-[10px] font-semibold border-b">
                <tr>
                  <th className="py-2">Proje</th>
                  <th className="py-2">Ad</th>
                  <th className="py-2">Durum</th>
                  <th className="py-2 text-right">Bütçe</th>
                ...
                {(data.projects || []).length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-6 text-center text-slate-400">
                      Proje yok. <button type="button" onClick={() => navigate("/projects")} className="text-emerald-700 underline">Proje oluştur</button>
                    </td>
                  </tr>
                )}
                  ...
                  return (
                    <tr key={p.id} className={p.status === "completed" ? "bg-emerald-50/40" : ""} data-testid={`detail-project-${p.project_number}`}>
                      <td className="py-2 font-mono font-semibold">
                        <button type="button" onClick={goToProject} className="hover:underline text-left" data-testid={`detail-project-open-${p.project_number}`}>{p.project_number}</button>
                      </td>
                      <td className="py-2">{p.name}</td>
                      <td className="py-2">
                        <div className="flex flex-col gap-0.5 items-start">
                          <span
                            className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${p.status === "completed" ? "bg-emerald-50 text-emerald-700" : p.status === "active" ? "bg-blue-50 text-blue-700" : p.status === "on_hold" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-600"}`}
                            data-testid={`detail-project-status-${p.project_number}`}
                          >
                            {statusTr(p.status)}
                          </span>
                      ...
                      <td className="py-2 text-right">
                        <div className="flex justify-end gap-1">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2 py-1 border border-emerald-200 text-emerald-700 bg-emerald-50 rounded-md text-[10px] font-semibold hover:bg-emerald-100"
                            data-testid={`detail-project-track-${p.project_number}`}
                          >
                            <Link2 className="w-3 h-3" /> Takip Linki
                          </button>
                          <button
                            type="button"
                            onClick={goToProject}
                            className="px-2 py-1 bg-slate-900 text-white rounded-md text-[10px] font-semibold"
                            title="Proje sayfasına git"
                            data-testid={`detail-project-goto-${p.project_number}`}
                          >
                            Projeye Git
                          </button>
                        </div>
