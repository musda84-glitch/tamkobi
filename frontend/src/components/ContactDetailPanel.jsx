              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {[...(data.projects || [])].sort((a, b) => Number(b.status === "completed") - Number(a.status === "completed")).map((p) => {
                  ...
                  <tr key={p.id} className={p.status === "completed" ? "bg-emerald-50/40" : ""} data-testid={`detail-project-${p.project_number}`}>
                    <td className="py-2 font-mono font-semibold"><button type="button" onClick={goToProject} className="hover:underline text-emerald-700" data-testid={`detail-project-open-${p.project_number}`}>{p.project_number}</button></td>
                    ...
                    <td className="py-2 text-right">
                      <div className="flex justify-end gap-1">
                        <button type="button" onClick={goToProject} className="inline-flex items-center gap-1 px-2 py-1 border rounded-md text-[10px] font-semibold hover:bg-slate-50" title="Proje sayfasına git" data-testid={`detail-project-goto-${p.project_number}`}><Briefcase className="w-3 h-3" /> Projeye Git</button>
                        <button type="button" onClick={() => setTrackingProject(p)} className="inline-flex items-center gap-1 px-2 py-1 border border-emerald-200 text-emerald-700 bg-emerald-50 rounded-md text-[10px] font-semibold" data-testid={`detail-project-track-${p.project_number}`}><Link2 className="w-3 h-3" /> Takip Linki</button>
                      </div>
                    </td>
                  </tr>
                  );
