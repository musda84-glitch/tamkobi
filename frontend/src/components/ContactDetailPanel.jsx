...
<button type="button" onClick={(e) => openInvCtxFromButton(e, inv)} className="p-1.5 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition" title={incoming ? "Gelen e-fatura işlemleri" : "Fatura kesim & diğer işlemler"} data-testid={`detail-inv-more-${inv.invoice_number}`}><MoreVertical className="w-4 h-4" /></button>
          ...
          {tab === "projects" && (
            <div className="space-y-3" data-testid="detail-projects-list">
              {(data.projects || []).length === 0 && (
                <div className="py-8 text-center text-slate-400 border border-dashed rounded-2xl">
                  Proje yok. <button type="button" onClick={() => navigate("/projects")} className="text-emerald-700 underline font-semibold">Proje oluştur</button>
                </div>
              )}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {[...(data.projects || [])].sort((a, b) => Number(b.status === "completed") - Number(a.status === "completed")).map((p) => {
                  ...
                  const done = (p.tasks || []).filter((t) => t.done || t.status === "done" || t.status === "completed").length;
                  const taskN = (p.tasks || []).length;
                  return (
                    <div
                      key={p.id}
                      className={`bg-white border rounded-2xl p-4 space-y-2 text-xs ${p.status === "completed" ? "border-emerald-200 bg-emerald-50/30" : "border-slate-200"}`}
                      data-testid={`detail-project-${p.project_number}`}
                    >
                      <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0">
                          <button type="button" onClick={goToProject} className="font-mono text-[10px] text-emerald-700 hover:underline font-semibold" data-testid={`detail-project-open-${p.project_number}`}>{p.project_number}{p.quote_number ? ` · ${p.quote_number}` : ""}</button>
                          <div className="font-bold text-slate-900 text-sm truncate">{p.name}</div>
                          {p.address ? <div className="text-slate-500 truncate">{p.address}</div> : null}
                        </div>
                        <span className={`shrink-0 px-2 py-0.5 rounded-md text-[10px] font-bold ${p.status === "completed" ? "bg-emerald-50 text-emerald-700" : p.status === "active" ? "bg-blue-50 text-blue-700" : p.status === "on_hold" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-600"}`} data-testid={`detail-project-status-${p.project_number}`}>{statusTr(p.status)}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-1 text-[10px]">
                        ...
                        <div className="bg-slate-50 rounded-lg p-1.5"><div className="text-slate-400">Masraf</div><b className={(p.expense_total || 0) > 0 ? "text-rose-700" : ""}>{fmt(p.expense_total)} ₺</b></div>
                      </div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <TrackingBadge project={p} />
                        {taskN > 0 ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 font-semibold border border-indigo-100">{done}/{taskN} görev</span> : null}
                      </div>
                      <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t">
                        <button type="button" onClick={goToProject} className="flex items-center gap-1 px-2.5 py-1.5 border border-slate-200 text-slate-800 bg-white hover:bg-slate-50 rounded-lg font-semibold" title="Proje sayfasına git" data-testid={`detail-project-goto-${p.project_number}`}><Briefcase className="w-3.5 h-3.5" /> Projeye Git</button>
                        <button type="button" onClick={() => setTrackingProject(p)} className="flex items-center gap-1 px-2.5 py-1.5 border border-emerald-200 text-emerald-700 bg-emerald-50 rounded-lg font-semibold" data-testid={`detail-project-track-${p.project_number}`}><Link2 className="w-3.5 h-3.5" /> Takip Linki</button>
                      </div>
                    </div>
                  );
                })}
