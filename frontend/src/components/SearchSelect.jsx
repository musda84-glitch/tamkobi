...
export const SearchSelect = ({ value, onChange, options, placeholder = "Ara...", getLabel, getSub, getExtra, getImage, testId, className = "" }) => {
  ...
  return (
    <div ref={ref} className={`relative ${className}`} data-testid={testId}>
      ...
      {open && (
        <div className={`absolute z-30 mt-1 w-full ${getExtra ? "min-w-[340px]" : "min-w-[280px]"} bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden`}>
          ...
          <div className="max-h-60 overflow-y-auto">
            ...
            {filtered.map((o) => (
              <button type="button" key={o.id} onClick={() => { onChange(o.id, o); setOpen(false); setQ(""); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left text-xs hover:bg-emerald-50 ${o.id === value ? "bg-emerald-50/60" : ""}`} data-testid={`${testId}-option-${o.id}`}>
                ...
                <div className="flex-1 min-w-0"><div className="font-semibold text-slate-900 truncate">{getLabel(o)}</div>{getSub && <div className="text-[10px] text-slate-500 truncate">{getSub(o)}</div>}{getExtra && <div className="text-[10px] text-amber-800 mt-0.5 leading-snug" data-testid={`${testId}-cost-${o.id}`}>{getExtra(o)}</div>}</div>
              </button>
            ))}
          </div>
        </div>
