...
export const SearchSelect = ({ value, onChange, options, placeholder = "Ara...", getLabel, getSub, getExtra, getImage, testId, className = "" }) => {
  ...
  const menu = open && box && typeof document !== "undefined" ? createPortal(
    <div
      ref={menuRef}
      className="fixed z-[90] bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden"
    ...
    >
      ...
      <div className="overflow-y-auto" style={{ maxHeight: Math.max(120, box.maxHeight - 40) }}>
        {filtered.length === 0 && <div className="p-3 text-xs text-slate-400 text-center">Sonuç yok</div>}
        {filtered.map((o) => (
          <button type="button" key={o.id} onClick={() => { onChange(o.id, o); setOpen(false); setQ(""); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left text-xs hover:bg-emerald-50 ${o.id === value ? "bg-emerald-50/60" : ""}`} data-testid={testId ? `${testId}-option-${o.id}` : undefined}>
