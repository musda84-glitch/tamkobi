...
export const RecipeModal = ({ companyId, products, recipe, presetProductId, onClose, onSaved }) => {
  ...
  const pickMat = (i, id) => { const p = products.find((x) => x.id === id); upd(i, { product_id: id, product_name: p?.name || "", unit: p?.unit || "Adet", cost_per_unit: p?.purchase_price || 0 }); };
  const matCost = mats.reduce((s, m) => s + Number(m.cost_per_unit || 0) * Number(m.quantity || 0) * (1 + Number(m.wastage_percent || 0) / 100), 0);
    ...
    <div className="fixed inset-0 z-[70] bg-slate-900/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-3xl w-full p-5 space-y-4 text-xs shadow-2xl max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} data-testid="recipe-modal">
        ...
        <div>
          ...
          <div className="grid grid-cols-12 gap-1 px-1 text-[10px] uppercase font-semibold text-slate-400"><div className="col-span-5">Hammadde</div><div className="col-span-2 text-center">Miktar</div><div className="col-span-1 text-center">Fire %</div><div className="col-span-2 text-right">Birim Maliyet</div><div className="col-span-2 text-right">Tutar</div></div>
          <div className="space-y-1.5">
            {mats.map((m, i) => (
              <div key={i} className="grid grid-cols-12 gap-1 items-center bg-slate-50 border border-slate-200 rounded-lg p-1.5" data-testid={`recipe-material-${i}`}>
                <div className="col-span-5"><SearchSelect value={m.product_id} options={materialsSrc.filter((p) => p.id !== f.finished_product_id)} placeholder="Hammadde ara…" getLabel={(p) => p.name} getSub={(p) => `${p.sku} • Stok: ${p.stock_quantity} ${p.unit}`} onChange={(id) => pickMat(i, id)} testId={`recipe-mat-select-${i}`} /></div>
                ...
                <div className="col-span-2"><input type="number" min="0" step="any" value={m.cost_per_unit} onChange={(e) => upd(i, { cost_per_unit: e.target.value })} className="w-full bg-white border rounded p-1.5 text-right" /></div>
                <div className="col-span-1 text-right font-bold">{fmt(Number(m.cost_per_unit || 0) * Number(m.quantity || 0) * (1 + Number(m.wastage_percent || 0) / 100))}</div>
        ...
        <div>
          ...
          {steps.length === 0 && <p className="text-[11px] text-slate-400">Adım tanımlanmazsa tek adımlı ("Üretim") iş emri oluşur. Örn: Kesim → Montaj → Kalite Kontrol → Paketleme</p>}
          <div className="space-y-1.5">{steps.map((st, i) => (
            <div key={i} className="grid grid-cols-12 gap-1 items-center bg-slate-50 border border-slate-200 rounded-lg p-1.5" data-testid={`recipe-step-${i}`}>
              ...
              <div className="col-span-5"><input value={st.name} onChange={(e) => updStep(i, { name: e.target.value })} placeholder="Adım adı (Kesim, Montaj…)" className="w-full bg-white border rounded p-1.5" data-testid={`recipe-step-name-${i}`} /></div>
              <div className="col-span-3"><input value={st.station} onChange={(e) => updStep(i, { station: e.target.value })} placeholder="İstasyon / Makine" className="w-full bg-white border rounded p-1.5" data-testid={`recipe-step-station-${i}`} /></div>
