...
const BizimHesapCard = ({ companyId, onImported }) => {
  ...
  const importCustomers = async () => { if (!window.confirm("BizimHesap carileri (ünvan, VKN, iletişim, bakiye) aktarılsın mı? Mevcut cariler VKN/ünvan eşleşmesiyle güncellenir; işlem Aktarım Günlüğü'nden geri alınabilir.")) return; setBusy("cust"); setErr(""); try { const r = await axios.post(`${API_URL}/migration/bizimhesap/import-customers`, { company_id: companyId, ...custOpt, on_duplicate: "update" }); toast.success(r.data.message); load(); onImported(); } catch (e) { const m = e.response?.data?.detail || "Cari aktarımı başarısız."; setErr(m); toast.error(m); } finally { setBusy(""); } };
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 text-xs" data-testid="bizimhesap-card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        ...
        <div className="flex gap-2"><button onClick={save} disabled={busy === "save" || (!token && !cfg?.configured && !firmId)} className="px-3 py-2 bg-slate-900 text-white rounded-lg font-semibold disabled:opacity-50" data-testid="bh-save">Kaydet</button><button onClick={runTest} disabled={!canTest} className="px-3 py-2 border border-slate-200 rounded-lg font-semibold flex items-center gap-1 disabled:opacity-50" data-testid="bh-test">{busy === "test" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plug className="w-3.5 h-3.5" />} Bağlantıyı Test Et</button></div>
      </div>
