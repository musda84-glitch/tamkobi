...
export const AiInvoiceImportModal = ({ companyId, contacts, onClose, onDone, invoiceType = "purchase" }) => {
  ...
  const setParty = (patch) => setDraft((d) => (
    isSales
      ? { ...d, customer: { ...(d.customer || {}), ...patch } }
      : { ...d, supplier: { ...(d.supplier || {}), ...patch } }
  ));
  ...
  const title = isSales
    ? "AI ile PDF / XML Satış Faturası Yükle"
    : "AI ile PDF Faturadan Alış Faturası Oluştur";
  const dropHint = isSales
    ? "Satış e-Fatura / e-Arşiv PDF veya UBL-TR XML sürükleyin veya seçin"
    : "Tedarikçi fatura PDF'ini sürükleyin veya seçin";
  const dropSub = isSales
    ? "Metin tabanlı PDF veya UBL-TR XML · max 10 MB. Müşteri, kalemler, KDV ve toplamlar otomatik çıkarılır."
          ...
          <div className="space-y-4 text-xs">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="md:col-span-2 bg-slate-50 rounded-xl p-3 space-y-2">
                <div className="font-bold text-slate-800 flex items-center gap-1.5"><FileText className="w-4 h-4" /> {isSales ? "Müşteri" : "Tedarikçi"}</div>
                <div className="grid grid-cols-2 gap-2">
                  <input value={party.name || ""} onChange={(e) => setParty({ name: e.target.value })} placeholder="Ünvan" className="border rounded-lg p-2 col-span-2" data-testid="ai-sup-name" />
                  <input value={party.tax_number || ""} onChange={(e) => setParty({ tax_number: e.target.value })} placeholder="VKN / TCKN" className="border rounded-lg p-2" data-testid="ai-sup-tax" />
                  <input value={party.tax_office || ""} onChange={(e) => setParty({ tax_office: e.target.value })} placeholder="Vergi Dairesi" className="border rounded-lg p-2" />
                </div>
                <div>
                  ...
                  <SearchSelect value={contactId} onChange={setContactId} options={contactOptions} getLabel={(c) => c.name} getSub={(c) => c.tax_number_or_id} placeholder={isSales ? "Mevcut müşteri seç — boş bırakırsanız yeni cari açılır" : "Mevcut tedarikçi seç — boş bırakırsanız yeni cari açılır"} testId="ai-contact-select" />
                </div>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 space-y-2">
                <div className="font-bold text-slate-800">Fatura Bilgileri</div>
                <input value={draft.invoice_number || ""} onChange={(e) => setDraft({ ...draft, invoice_number: e.target.value })} placeholder={isSales ? "Fatura no" : "Tedarikçi fatura no"} className="w-full border rounded-lg p-2" data-testid="ai-inv-no" />
                ...
                <div className="text-[11px] text-slate-500">Güven: <b>%{Math.round((draft.confidence || 0) * 100)}</b> · Belge toplamı: <b>{fmt(draft.grand_total)} {draft.currency || "TRY"}</b>{res?.source === "xml" && <span className="ml-1 text-violet-700 font-semibold">· UBL XML</span>}</div>
              </div>
            </div>
