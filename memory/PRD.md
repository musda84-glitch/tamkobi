...
## İterasyon 39 (Eylül 2026) ✅ test ajanı: backend 15/15, frontend %100 (`saas_extras.py`)
- **Şirket Olarak Gir (destek modu)**: `POST /api/system/companies/{id}/impersonate` → hedef şirketin admini için 2 saatlik token (`imp_by` claim), `sa_return` çerezinde süper admin oturumu saklanır; `/auth/me` → `impersonation`; ERP'de `impersonation-banner` + "Destek modunu bitir" (`POST /api/auth/impersonate/exit`) → panele döner. Panelde Şirketler satırında "Şirket Olarak Gir". activity_logs'a IMPERSONATE kaydı.
- **Otomatik e-Arşiv fatura**: her başarılı abonelik ödemesinde (`_apply_payment`) platform şirketi (Platform Ayarları → gönderici şirket) adına `ABN{yıl}{sıra}` numaralı, KDV dahil (%20 içinden ayrıştırılır), ödendi durumunda e-Arşiv satış faturası + müşteri şirket için "Abonelik Müşterisi" carisi; müşteri yöneticisine HTML e-posta (SMTP) — gerçek gönderim doğrulandı. Ödeme sonucu sayfasında ve panel Ödemeler'de fatura no.
- **PayTR iFrame** sağlayıcısı: Platform Ayarları → PayTR (merchant_id, key/salt Fernet şifreli, test modu, taksit, aktif); `GET /api/payments/providers`; `POST /api/payments/paytr/session` (hash, sepet base64, TL, ok/fail URL) → iframe modal (MyPlanPanel "PayTR ile Öde"); `POST /api/payments/paytr/callback` (HMAC doğrulama, idempotent, "OK") → `_apply_payment`. Bildirim URL'si: `{origin}/api/payments/paytr/callback`. **Kullanıcının PayTR mağaza bilgileri henüz girilmedi** (aktif değil).
- İter. 39b: PayTR **Bağlantıyı Test Et** (`POST /api/system/paytr/test` → gerçek get-token isteği, sonuç `paytr.last_test`), kurulum kontrol listesi + callback URL kopyala; Toggle bileşeni etiket çakışması düzeltildi. **PayTR mağaza bilgileri kullanıcı tarafından hâlâ girilmedi** (kullanıcı ask_human'ı atladı).

## İterasyon 40–41 (Eylül 2026) ✅ test ajanı: 40 → backend 9/9 + frontend %100; 41 → backend 10/10 + frontend %100
- **Fatura PDF eki** (`saas_docs.py`, reportlab): abonelik e-Arşiv faturası PDF olarak e-postaya eklenir; `GET /api/invoices/{id}/pdf`; ödeme sonucu sayfasında "Fatura PDF".
- **Yenileme linki**: hatırlatma e-posta/WhatsApp'ında `/yenile/<JWT 30 gün>` → `RenewPage` (paket + dönem + Stripe/PayTR) → `GET/POST /api/public/renew/{token}[/checkout]`; Platform Ayarları `public_url` (+ `.env PUBLIC_APP_URL`).
- **Görsel düzenlemeler**: Siparişler işlemler ⋮ menüsü (5 ikon), sipariş ürün satırı → `/stock?q=`; Yeni Fatura: Tevkifat (GİB kodları, `withholding_*` backend hesaplı), Ü/H hizmet satırı (`is_service`), KDV Hariç/Dahil fiyat modu, Taslak/Onaylı (varsayılan kağıt+taslak); Yazdırma şablonu: satır fiyat/KDV/tüm fiyat gizle, satır notu, sipariş notu; termal etikette fiyat yok; şablon kaydında önizleme canlı yenilenir.
- **B2B**: ayrı giriş `/b2b/giris` (`POST /api/public/b2b/login`: e-posta/VKN + şifre → `/portal/{token}`); şifre cari kartı → B2B Portal sekmesi (`b2b_password`, `b2b_login_email`) veya `POST /contacts/{id}/b2b-access {password}`. **AI sepet**: portalda Excel/PDF yükle → `POST /api/public/b2b/{token}/ai-cart` (Claude + SKU/barkod/ad fuzzy eşleme) → öneri modalı → sepete ekle. Örnek: `backend/tests/b2b_siparis_ornek.xlsx`.
- **Gelişmiş Cari Formu** (`ContactForm.jsx`, 6 sekme; Contact modeline contact_person, iban, tags, risk_status, currency, payment_method, sales_rep, opt-in alanları) — Yeni Cari + satır düzenle (kalem). `clean_doc` artık password hash'lerini gizler (`has_b2b_password`).
- **Üst bar**: genel arama (`GET /api/search`, Ctrl+K) + Yeni Cari + Yeni Sipariş (`/orders?new=1`).
- İter. 41b: Cari detay → Faturalar sekmesinde taslak faturalar listelenir ama bakiye dışı: özet şeridi ("N onaylı · tutar (bakiyeye işlendi)" / "N taslak · tutar (bakiyeye işlenmez)"), taslak satırları amber kenarlık + "Taslak · bakiye dışı" etiketi. Backend zaten yalnız approved faturayı bakiyeye işler.
- İter. 41c: Sol menüde **B2B Portal Yönetimi** (`/b2b-yonetim`, `B2BAdminPage`; izin/lisans /contacts'a bağlı): müşteri giriş adresi kartı (kopyala/aç), 3 adımlı rehber, `B2BSettings` (SettingsPage'den export) + müşteri tablosunda "Portal Girişi" sütunu ve **Şifre Tanımla** modalı (`b2b-access` password/login_email), son giriş; aktifler önce, ilk 60 satır. Siparişler: ürün görselleri her zaman görünür, çoklu ürün "+N ürün daha · büyüt" ile açılır.
- İter. 41d: Faturalar listesinde taslaklar için **Düzenle** (modal önceden dolu → `PUT /invoices/{id}`, tevkifat dahil yeniden hesap), **Onayla** (`POST /invoices/{id}/approve` → `_apply_invoice_effects`: bakiye + stok), **Sil** (`DELETE /invoices/{id}` → yalnız taslak, çöp kutusuna). KRİTİK DÜZELTME: taslak fatura GİB'e gönderilince/onaylanınca bakiye ve stok artık işleniyor (`effects_applied` bayrağı; eskiden hiç işlenmiyordu); taslak düzenlemesi bakiyeyi değiştirmez. Curl ile bakiye akışı doğrulandı (taslak/edit 0 etki, onay +2200).
