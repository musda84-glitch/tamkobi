    ...
    <Screen>
      ...
      {hits?.contacts?.length ? (
        <Card>
          ...
          {hits.contacts.map((c) => (
            <ListRow key={idOf(c)} title={c.name} right={fmtMoney(c.balance)} onPress={() => go("ContactDetail", { id: idOf(c), name: c.name })} />
          ))}
        </Card>
      ) : null}
      ...
      {hits?.invoices?.length ? (
        <Card>
          ...
          {hits.invoices.map((i) => (
            <ListRow key={idOf(i)} title={i.invoice_number || "Fatura"} subtitle={i.contact_name} right={fmtMoney(i.grand_total)} onPress={() => go("InvoiceDetail", { id: idOf(i) })} />
          ))}
        </Card>
      ) : null}
    </Screen>
