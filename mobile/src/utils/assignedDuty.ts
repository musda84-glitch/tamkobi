...
export function photoVisibilityShort(p?: DutyPhoto | null): string {
  const vis = photoVisibility(p);
  return vis === "show" ? "Görür" : vis === "hide" ? "Görmez" : "Onay";
}

export function photoVisibilityIcon(p?: DutyPhoto | null): "eye" | "eye-off" | "time-outline" {
  const vis = photoVisibility(p);
  return vis === "show" ? "eye" : vis === "hide" ? "eye-off" : "time-outline";
}
