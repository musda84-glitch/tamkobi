...
export function StaffMessagesPanel({
  ...
  const inboxUnread = inboxUnreadTotal(data?.inbox);

  if (error && !data) {
    return (
      <View testID="home-messages-panel" style={{ padding: 12, borderRadius: 20, borderWidth: 1, borderColor: tone.border, backgroundColor: tone.bg }}>
      ...
      {showStaff ? (
        <View style={{ gap: 8 }}>
          ...
          >
            ...
            <Text style={{ fontWeight: "800", color: colors.text, fontSize: 13, flex: 1 }}>Yönetici mesajları</Text>
            {unread ? (
              <View style={{ paddingHorizontal: 7, paddingVertical: 2, borderRadius: 999, backgroundColor: colors.danger }}>
                <Text testID="home-messages-unread" style={{ color: "#fff", fontSize: 10, fontWeight: "800" }}>{unread} yeni</Text>
              </View>
            ) : null}
          </Pressable>
          {!preview.length ? (
            <View style={{ backgroundColor: colors.surface, borderRadius: 14, paddingVertical: 12, alignItems: "center" }}>
              <Text style={{ color: colors.muted, fontSize: 12 }}>Yöneticiden henüz mesaj yok.</Text>
            </View>
          ) : preview.map((m) => (
            <View
      ...
      {showInbox ? (
        <View style={{ gap: 6 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <Text style={{ fontWeight: "800", color: colors.text, fontSize: 13, flex: 1 }}>Personel mesajları</Text>
            {inboxUnread ? (
              <View style={{ paddingHorizontal: 7, paddingVertical: 2, borderRadius: 999, backgroundColor: colors.danger }}>
                <Text style={{ color: "#fff", fontSize: 10, fontWeight: "800" }}>{inboxUnread} yeni</Text>
              </View>
            ) : null}
          </View>
          {!(data.inbox || []).length ? (
            <Muted>Personelden henüz mesaj yok. Personel kartından yazabilirsiniz.</Muted>
          ) : (data.inbox || []).slice(0, 5).map((row) => (
            <Pressable
              key={row.employee_id}
            ...
            >
              ...
              <Text style={{ color: colors.muted, fontSize: 11 }} numberOfLines={1}>{messagePreview(row.last)}</Text>
            </Pressable>
          ))}
        </View>
      ) : null}
