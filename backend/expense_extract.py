...
async def extract_expense_file(data: bytes, filename: str = "", content_type: str = "") -> dict:
    ...
    if is_img:
        ...
        draft = await extract_expense_from_image(data, mime, filename)
...
async def extract_expense_from_image(data: bytes, mime: str = "image/jpeg", filename: str = "") -> dict:
    ...
    raise ValueError("Fişten tutar okunamadı. Daha net bir fotoğraf deneyin.") from last_err
