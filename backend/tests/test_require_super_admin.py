...
def test_tam_bos_bearer_demo_yoluna_dusmez(monkeypatch):
    # Sömürülebilir tek girdi başlığın tam olarak "Bearer " olması. Eski kap
    # yalnızca öneke bakıyordu, `get_current_user` ise öneki kesip boş token
    # görünce isteği oturumsuz sayıp tohum `admin@nexus.com`'a düşüyordu — o
    # kayıt `is_super_admin` olduğu için sistem paneli (veritabanı taşıma
    # dahil) tamamen açılıyordu. Bir boşluk daha olsa token boş olmaz ve eski
    # kod da reddederdi; gerilemeyi yalnızca bu girdi yakalıyor.
    async def demo(_request):
        return {"is_super_admin": True, "email": "admin@nexus.com"}
