...
class TestPricing:
    ...
    def test_compute_shopphp_push_supported_rest_config(self, sess):
        # Gönderim desteği ShopPHP'de REST kullanıcısının tanımlı olmasına
        # bağlı; fiyat hesabı da aynı bayrağı taşımalı. Eskiden koşulsuz False
        # bekleniyordu, bu kural gelmeden önceki haldi.
        cfgs = sess.get(f"{BASE}/api/integrations/ecommerce", params={"company_id": CID}).json()
        cfg = next((c for c in cfgs if c.get("channel") == "shopphp"), None)
        r = sess.post(f"{BASE}/api/pricing/compute", json={"company_id": CID, "channel": "shopphp"})
        assert r.status_code == 200
        assert r.json().get("push_supported") is bool(cfg and cfg.get("rest_configured"))
