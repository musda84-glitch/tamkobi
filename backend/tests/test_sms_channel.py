...
def test_approval_dispatch_summary_sms_and_email_failed():
    assert approval_dispatch_summary({"sms": {"status": "failed", "detail": "Geçersiz GSM numarası"}}) == {
        "status": "failed",
        "message": "SMS başarısız: Geçersiz GSM numarası",
    }
    assert approval_dispatch_summary({"email": {"status": "failed", "detail": "E-posta hesabı tanımlı değil."}})["status"] == "failed"
    mixed = approval_dispatch_summary({
        "sms": {"status": "failed", "detail": "Netgsm 40"},
        "email": {"status": "failed", "detail": "SMTP yok"},
        "whatsapp": {"status": "sent"},
    })
    assert mixed["status"] == "success"
    assert "SMS başarısız" in mixed["message"]
    assert "E-posta başarısız" in mixed["message"]
