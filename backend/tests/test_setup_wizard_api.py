...
class TestSetupWizardApi:
    ...
    def test_test_db_accepts_live_mysql(self):
        status = requests.get(f"{API}/setup/status", timeout=20).json()
        d = status["defaults"]
        r = requests.post(
            f"{API}/setup/test-db",
            json={
                ...
                "db_name": d["db_name"],
                "db_user": d["db_user"],
                "db_password": os.environ.get("MYSQL_PASSWORD") or os.environ.get("DB_PASSWORD") or "tamkobi",
            },
            timeout=20,
        )
        assert r.status_code == 200, r.text
        assert r.json()["ok"] is True
        assert r.json()["database"]
