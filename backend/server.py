...
async def list_contacts(company_id: Optional[str] = "comp_nexus_main_01", type: Optional[str] = None, lite: bool = False):
    ...
    if not lite:
        try:
            ...
            logging.exception("contact open amounts")
    return docs
...
async def _contact_open_amounts(company_id: Optional[str]) -> dict:
    ...
    for r in rows:
        ...
        if not cid:
            continue
        try:
            out[str(cid)] = round(float(r.get("invoiced") or 0) - float(r.get("paid") or 0), 2)
        except (TypeError, ValueError):
            continue
    return out
...
async def b2b_create_order(token: str, req: Dict[str, Any]):
    ...
    return {"status": "success", "order": clean_doc(refreshed), "message": f"Siparişiniz alındı: {doc['order_number']}"}
...
async def create_expense_slip_from_invoice(invoice_id: str):
    ...
    doc = {
        ...
        "withholding_rate": inv.get("withholding_rate") or 0,
        "withholding_code": inv.get("withholding_code"),
        "withholding_amount": inv.get("withholding_amount") or 0,
...
async def update_order(order_id: str, req: Dict[str, Any]):
    ...
    if update.get("items"):
        refreshed = {**o, **update}
        await _sync_draft_invoice_items(refreshed, update["items"])
    updated = await db.orders.find_one({"_id": order_id})
...
async def _attach_draft_invoice_on_intake(order: dict, source: str = "intake") -> Optional[dict]:
    """Yeni gelen siparişi cariye taslak fatura olarak düşür (hata siparişi bozmasın)."""
    try:
        return await _create_draft_invoice_for_order(order, source=source)
    except Exception:
        logging.getLogger(__name__).exception(
            "Sipariş alışında taslak fatura oluşturulamadı: %s", (order or {}).get("_id")
        )
        return None


async def _sync_draft_invoice_items(order: dict, items: list) -> None:
    """Bekleyen sipariş kalemleri değişince bağlı taslak faturayı güncelle."""
    if not order or not order.get("invoice_id") or order.get("is_invoiced") or not items:
        return
    invoice = await db.invoices.find_one({"_id": order["invoice_id"]})
    if not invoice or invoice.get("status") != "draft":
        return
    inv_rows = order_items_to_invoice_items(items)
    await _fill_stock_codes(order.get("company_id"), inv_rows)
...
async def upsert_attendance(req: Dict[str, Any]):
    ...
    edit = round_info.get("edit")
    if edit:
        auto = bool(round_info.get("auto_confirm"))
        rec["manager_time_edit"] = edit
        rec["manager_time_edit_rounds"] = round_info.get("rounds") or {}
        rec["employee_confirmed"] = auto
        rec["employee_confirmed_at"] = attendance._now() if auto else None
        rec["message"] = attendance.manager_time_edit_result_message(round_info)
        await db.attendance.update_one(
            {"employee_id": emp["_id"], "date": date},
            {"$set": {
                "manager_time_edit": edit,
                "manager_time_edit_rounds": rec["manager_time_edit_rounds"],
                "employee_confirmed": auto,
                "employee_confirmed_at": rec["employee_confirmed_at"],
            }},
        )
        if not auto:
            try:
                ...
                await _notify.insert_notification(db, _notify.notification_doc(
                    ...
                    f"Yönetici giriş/çıkış saatini güncelledi ({edit.get('prev_check_out') or edit.get('prev_check_in') or '—'} → {edit.get('check_out') or edit.get('check_in') or '—'}). Personel onayı gerekir ({round_info.get('attempts')}/3).",
                    link="/mesai",
                    user_id=emp.get("user_id"),
...
async def create_order(order: Order):
    ...
    await _attach_draft_invoice_on_intake(doc, source="intake")
    refreshed = await db.orders.find_one({"_id": doc["_id"]}) or doc
...
async def ai_order_confirm(req: Dict[str, Any]):
    ...
    for o in req.get("orders") or []:
        ...
        await db.orders.insert_one(doc)
        await _ensure_order_contact(doc)
        await _attach_draft_invoice_on_intake(doc, source="import")
        refreshed = await db.orders.find_one({"_id": doc["_id"]}) or doc
        created.append(clean_doc(refreshed))
...
@api_router.post("/ai/cheque-extract")
async def ai_cheque_extract(
    request: Request,
    file: UploadFile = File(...),
    company_id: str = Query("comp_nexus_main_01"),
    user: dict = Depends(get_current_user),
    ...
    name = file.filename or "cheque.jpg"
    data = await file.read()
    if len(data) > 10 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Dosya en fazla 10 MB olabilir.")
    try:
        out = await extract_cheque_file(data, name, file.content_type or "")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)[:220]) from e
    except Exception as e:
        logger.error("AI cheque extract failed: %s", e)
        err = str(e)
        hint = ""
        low = err.lower()
        if "401" in err or "403" in err or "anahtar" in low or "api key" in low:
    ...
    if not match and draft.get("drawer_name"):
        match = await db.contacts.find_one({
            "company_id": company_id,
            "name": {"$regex": re.escape(str(draft["drawer_name"])[:25]), "$options": "i"},
        })
    return {
        "draft": draft,
        "source": out.get("source"),
        "matched_contact": public_cheque_match(match),
        "filename": name,
        "text_preview": out.get("text_preview") or "",
    }
...
import platform_notices
platform_notices.init(db)
import user_erase
user_erase.init(db)
...
trash.init(db)
import mobile_logs
mobile_logs.init(db, get_current_user)
migration.init(db)
...
app.include_router(mobile_logs.router)
app.include_router(migration.router)
app.include_router(pricing.router)
app.include_router(edocs.router)
app.include_router(saas.router)
app.include_router(platform_notices.router)
