...
async def create_bonus(req: Dict[str, Any]):
    ...
    labels = {"bonus": "Prim", "second_salary": "İkinci Maaş", "advance": "Avans", "expense": "Masraf Ödemesi", "overtime": "Fazla Mesai", "yevmiye": "Yevmiye", "alacak": "Alacak", "borc": "Borç"}
...
async def _employee_receivable(emp: dict, payrolls: list, bonuses: list, month: str, expenses: Optional[list] = None) -> dict:
    ...
    bonus_pending = round(sum(_emp_num(b.get("amount")) for b in bonuses if b.get("type") not in ("advance", "borc") and b.get("status") != "paid"), 2)
