...
async def _company_row(c: dict) -> Dict[str, Any]:
    ...
    admin = await _db.users.find_one({"company_ids": c["_id"], "role": "admin", "is_super_admin": {"$ne": True}}, {"email": 1, "name": 1, "last_login_at": 1}) or await _db.users.find_one({"company_ids": c["_id"], "role": "admin"}, {"email": 1, "name": 1, "last_login_at": 1})
    ...
    return {"id": c["_id"], "name": c.get("name"), "tax_number": c.get("tax_number"), "city": c.get("city"), "phone": c.get("phone"), "email": c.get("email"), "created_at": c.get("created_at"), "license_id": lic.get("license_id") or c["_id"], "license_companies": siblings, "admin": {"email": admin.get("email"), "name": admin.get("name"), "last_login_at": admin.get("last_login_at")} if admin else None, "license": lic, "usage": await _usage(c["_id"])}
...
async def my_license(request: Request, company_id: str = "comp_nexus_main_01"):
    ...
    return {**lic, "catalog": catalog(), "plans": plans, "users": await _db.users.count_documents({"company_ids": company_id}), "companies": sibs, "pending_request": _clean(pending) if pending else None}
